...
import com.bill.bugexpert.settings.Setting;
...
/* package */ class Gui extends JFrame implements OutputListener, ActionListener {
    ...
    public Gui(Main main) {
        super("Bugexpert - (C) 2018 Bill.Huang");

        Image image=Toolkit.getDefaultToolkit().getImage("res/app_icon.png");
		setIconImage(image);

        mMain = main;
        mMod = (BugExpertModule) mMain.getModule();
    ...
    @Override
    public void log(final int level, final int type, final String msg) {
        if (level <= 1) {
