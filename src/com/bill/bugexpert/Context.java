...
/**
 * The Context contains various configurations which affects the whole
 * processing.
 */
public final class Context {
	...
	/**
	 * Changes the url which will be used as ChkBugReport's homepage. This could be
	 * used to redirect the link to internal website in organizations.
	 * 
	 * @param url
	 *            The new url
	 */
	...
	/**
	 * Returns the starting point of the time window to limit the logs to. If this
	 * value is set, the plugins processing the logs should ignore every line who's
	...
	/**
	 * Returns the ending point of the time window to limit the logs to. If this
	 * value is set, the plugins processing the logs should ignore every line who's
	...
	/**
	 * Returns the GMT offset/timezone of the logs. It's used to map the kernel
	 * timestamps with the log timestamps.
	 * 
	...
	/**
	 * Return the input file limitation. Note: this is just a recomended value, some
	 * sections might ignore this
	 * 
	 ...
	 * 
	 * @param value
	 *            The new file size limit in bytes
	 */
	 ...
	 * 
	 * @param silent
	 *            True if silent mode should be used
	 */
	public void setSilent(boolean silent) {
		mSilent = silent;
	}

	void parseTimeWindow(String timeWindow) {
		try {
	...
	void parseGmtOffset(String param) {
		try {
			if (param.startsWith("+")) {
	...
	public void logI(String s) {
		...
		if (mOutListener != null) {
			mOutListener.log(1, OutputListener.TYPE_OUT, s);
		}
	}
	...
	public void logD(String s) {
		...
		if (mOutListener != null) {
			mOutListener.log(2, OutputListener.TYPE_OUT, s);
		}
	}
	...
	public void logW(String s) {
		...
		if (mOutListener != null) {
			mOutListener.log(3, OutputListener.TYPE_OUT, s);
		}
	}
	...
	public void logE(String s) {
		...
		if (mOutListener != null) {
			mOutListener.log(4, OutputListener.TYPE_ERR, s);
		}
	}
	
	void setLogOutput(String logName) {
		if (mOut == null) {
			try {
