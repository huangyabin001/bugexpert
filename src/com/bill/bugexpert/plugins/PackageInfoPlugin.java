...
public class PackageInfoPlugin extends Plugin {
	...
	public void load(Module br) {
		mMod.logD("PackageInfoPlugin:load(),loading...");
		// Load packages.xml
		Section s = br.findSection(Section.PACKAGE_SETTINGS);
		if (s == null) {
			br.logE(TAG + "Cannot find section: " + Section.PACKAGE_SETTINGS
					+ ", new android version has no packages.xml in bugreport.txt.");
			return;
		}

		if (s.getLineCount() == 0 || s.getLine(0).startsWith("***")) {
			br.logE(TAG + "Cannot parse section: " + Section.PACKAGE_SETTINGS);
			return;
		}
