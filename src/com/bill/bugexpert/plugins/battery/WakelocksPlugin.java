...
import com.bill.bugexpert.doc.ChapterHelp;
...
public class WakelocksPlugin extends Plugin {
    ...
    public void load(Module br) {
        ...
        if (line.equals("*** /proc/wakelocks: No such file or directory")) {
            br.logE(TAG + "No data found in section: " + Section.KERNEL_WAKELOCKS + " (aborting plugin)");
            return;
        }
        ...
        if (!ok) {
            br.logE(TAG + "Data format changed in section " + Section.KERNEL_WAKELOCKS + " (aborting plugin)");
            return;
        }
