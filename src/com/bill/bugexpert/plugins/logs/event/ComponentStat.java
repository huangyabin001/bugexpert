...
public class ComponentStat {
    ...
    private void checkOnCreate(long ts) {
        ...
        if (_createTime == 0) {
            if (_createGuessed) {
                _br.logE("ComponentStat: onCreate was already guessed, but it's missing again (" + component + ")");
                onCreate(ts);
                errors++;
            } else {
                _br.logE("ComponentStat: onCreate was missing (" + component + ")");
                onCreate(_firstTs);
            }
            _createGuessed = true;
        }
    ...
    private void onResume(long ts) {
        ...
        if (_resumeTime == 0) {
            ...
            _br.logE("ComponentStat: onResume was called again without onPause (" + component + ")");
            errors++;
        }
    }
    ...
    private void onPause(long ts) {
        ...
        if (_resumeTime == 0) {
            if (_resumeGuessed) {
                _br.logE("ComponentStat: onResume was already guessed, but it's missing again (" + component + ")");
                onResume(ts);
                errors++;
            } else {
                _br.logE("ComponentStat: onResume was missing (" + component + ")");
                onResume(_firstTs);
            }
            _resumeGuessed = true;
        }
        ...
        if (_resumeTime != 0) {
            ...
            if (time < 0) {
                _br.logE("ComponentStat: negative resumed time found, there is some problem with the timestamps!");
                errors++;
            }
    ...
    private void onCreate(long ts) {
        if (_createTime == 0) {
            ...
            _br.logE("ComponentStat: onCreate was called again without onDestroy (" + component + ")");
            errors++;
        }
    }
    ...
    private void onDestroy(long ts) {
        ...
        if (_createTime != 0) {
            ...
            if (time < 0) {
                _br.logE("ComponentStat: negative created time found, there is some problem with the timestamps!");
                errors++;
            }
