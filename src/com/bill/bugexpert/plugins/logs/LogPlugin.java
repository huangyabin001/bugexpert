...
import com.bill.bugexpert.TimeWindowMarker;
...
import com.bill.bugexpert.chart.Marker;
...
public abstract class LogPlugin extends Plugin implements LogData {
    ...
    public void load(Module rep) {
        ...
        if (mSection == null) {
            br.logE(TAG + "Cannot find section " + mSectionName + " (aborting plugin)");
            return;
        }
    ...
    protected void onLoaded(BugExpertModule br) {
        // NOP
    }
    ...
    private Chapter generateLog(BugExpertModule br) {
    ...
    private void generateSpamTopList(BugExpertModule br, Chapter mainCh) {
    ...
    protected ProcessLog getLogOf(BugExpertModule br, int pid) {
    ...
    private void reportFatalLog(LogLine sl, int i, BugExpertModule br, Section s) {
        // Put a marker box
    ...
    private int generateGCGraphs(BugExpertModule br, Chapter ch) {
        int cnt = 0;
    ...
    private boolean generateGCGraph(BugExpertModule br, Chapter ch, GCRecords gcs) {
    ...
    public void autodetect(Module mod, byte[] buff, int offs, int len, GuessedValue<String> type) {
        ...
        while ((line = lr.readLine()) != null) {
            try {
                LogLine sl = new LogLine((BugExpertModule) mod, line, fmt, prev);
                count++;
                if (sl.ok) {
                    okCount++;
                    fmt = sl.fmt;
