...
/* package */ class NewParser extends Parser {
    ...
    public NewParser(BugExpertModule br, FTracePlugin plugin) {
        mBr = br;
    }
    ...
    public FTraceData parse(Section section) {
        ...
        if (cnt <= 4) {
            mBr.logE(FTracePlugin.TAG + "The trace buffer is empty!");
            return null;
        }
        ...
        for (int i = 0; i < cnt; i++) {
            ...
            if (dstProc == null) {
                mBr.logE(FTracePlugin.TAG + "Cannot parse line, I better abort missiong!: " + buff);
                return null;
            }
            ...
            if (newNr <= 0) {
                ...
                mBr.logE(FTracePlugin.TAG + "Needs adjusting! newNr=" + newNr + " @" + timeUS);
                newNr = 1;
            }
            nrRunWait = newNr;
        ...
        mBr.logE(FTracePlugin.TAG + "WARNING: parsing of trace data with new format is still EXPERIMENTAL!");
        return ret;
    }
