...
import com.bill.bugexpert.AndroidVersions;
...
import com.bill.bugexpert.plugins.logs.LogPlugin;
import com.bill.bugexpert.util.ColorUtil;
...
public class SurfaceFlingerPlugin extends Plugin {
    ...
    public void load(Module rep) {
        ...
        if (sec == null) {
            br.logE(TAG + "Section not found: " + Section.DUMP_OF_SERVICE_SURFACEFLINGER + " (aborting plugin)");
            return;
        }
    ...
    private boolean scan(BugExpertModule br, Section sec) {
        int count = 0;
        int expectedCount = 0;
        int line = 0;
        ...
        if (buff.startsWith("Visible layers")) {
            ...
            if (idx0 < 0 || idx1 < 0 || idx0 >= idx1 + 2) {
                br.logE(TAG + "Error parsing: cannot find 'count' in first line!");
                return false;
            }
            ...
            br.logE(TAG + "Error parsing: cannot recognize section!");
            return false;
        }
        ...
        if (expectedCount > 0 && expectedCount != count) {
            br.logE(TAG + "Error parsing: count mismatch! Expected: " + expectedCount + ", found: " + count);
            return false;
        }
        ...
        if (found) {
            ...
            try {
                while (line < sec.getLineCount()) {
                    ...
                    if (buff.charAt(38) == '|') {
                        ...
                        if (m.matches()) {
                            ...
                            br.logE("Cannot parse buffer: " + buffer);
                        }
                    }
                    mBuffers.add(buffer);
                }
                ...
                // Ignore it for now
                br.logE(TAG + "Error parsing buffer list: " + nfe);
            } catch (StringIndexOutOfBoundsException e) {
                // Ignore it for now
                br.logE(TAG + "Error parsing buffer list: " + e);
            }
        }
    ...
    private int readRegion(Module br, Layer layer, String buff, Section sec, int line) {
        ...
        if ("transparentRegion".equals(type)) {
            ...
            br.logE("Warning: Unknown region: " + type);
            reg = new Region(type);
    ...
    private void readAttributes(BugExpertModule br, Layer layer, String buff) {
        ...
        while (tok.hasMoreTokens()) {
            ...
            if ("z".equals(key)) {
                ...
                if (!mUnknownAttrs.contains(key)) {
                    ...
                    br.logE(TAG + "Unknown layer attribute (this could brake parsing): " + key);
                }
