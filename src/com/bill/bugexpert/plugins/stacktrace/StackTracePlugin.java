 ...
 *
 * <p>
 * If DB access is working (in other words the sqlite jdbc driver is in the
 * classpath), then the stacktraces will be imported also in the database.
 * </p>
 *
 * <p>
...
public final class StackTracePlugin extends Plugin {
	...
	public void load(Module rep) {
		...
		// Load data
		run(br, ID_NOW, Section.VM_TRACES_JUST_NOW, "VM traces just now");
		run(br, ID_ANR, Section.VM_TRACES_AT_LAST_ANR, "VM traces at last ANR");

		// List all "VM TRACES WHEN SLOW" sections
		int id = ID_SLOW;
		for (Section sec : br.getSections()) {
			if (sec.getShortName().equals(Section.VM_TRACES_WHEN_SLOW)) {
				String ss = sec.getName();
				Matcher m = Pattern.compile("\\((.*)\\)").matcher(ss);
				if (m.find()) {
					String s = m.group(1);
					run(br, id++, Section.VM_TRACES_WHEN_SLOW, s);
				}
			}
		}
		...
		if (proc == null) {
			br.logE(TAG + "Cannot find section current stacktrace, ignoring binder states");
		} else if (sec == null) {
			br.logE(TAG + "Cannot find section " + Section.BINDER_STATE + ", ignoring it");
		} else {
	...
	private void run(BugExpertModule br, int id, String sectionName, String chapterName) {
		Section sec = br.findSection(sectionName);
		if (sec == null) {
			br.logE(TAG + "Cannot find section: " + sectionName + " (aborting plugin)");
			return;
		}
		// Scan stack traces
		...
		// Also do some initial pre-processing, mainly to extract some useful info for
		// other plugins
	...
	public void addSlowChapter(BugExpertModule br, Chapter main) {
		if (mSlowChapters == null) {
	...
	public void autodetect(Module module, byte[] buff, int offs, int len, GuessedValue<String> type) {
		String patterns[] = { "----- pid [0-9]+ at .* -----", "----- end [0-9]+ -----", "Cmd line: .*",
				"\".*\" sysTid=[0-9]+", "  #[0-9]+  pc [0-9a-f]+  .*", "DALVIK THREADS:", "\".*\" prio=.* tid=.* .*",
				"  \\| .*", "  at .*", };
		...
		if (okCount > 5 && okCount > count * 0.75f) {
			// We got a match, the only thing left is to detect if it's the event log or
			// system log
