...
/* package */ final class BinderAnalyzer {
    ...
    public void analyze(BugExpertModule br, Processes proc, Section sec) {
        ...
        for (int idx = 0; idx < cnt; idx++) {
            ...
            try {
                ...
                if (srcProc == null) {
                    br.logE(StackTracePlugin.TAG + "Cannot find process with pid " + srcPid + "!");
                    continue;
                }
                StackTrace srcThread = srcProc.findPid(srcTid);
                if (srcThread == null) {
                    br.logE(StackTracePlugin.TAG + "Cannot find thread with pid " + srcTid + "!");
                    continue;
                }
                Process dstProc = proc.findPid(dstPid);
                if (dstProc == null) {
                    br.logE(StackTracePlugin.TAG + "Cannot find process with pid " + dstPid + "!");
                    continue;
                }
                StackTrace dstThread = dstProc.findPid(dstTid);
                if (dstThread == null) {
                    br.logE(StackTracePlugin.TAG + "Cannot find thread with pid " + dstTid + "!");
                    continue;
                }
