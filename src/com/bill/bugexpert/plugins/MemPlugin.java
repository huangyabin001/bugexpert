...
import com.bill.bugexpert.ImageCanvas;
...
import com.bill.bugexpert.doc.Img;
...
public class MemPlugin extends Plugin {
	...
	public void load(Module mod) {
		mMod.logD("MemPlugin:load(),loading...");
	...
	public void generate(Module mod) {
		...
		if (sec == null) {
			mod.logE(TAG + "Section not found: " + Section.MEMORY_INFO + " (ignoring section)");
		} else {
		...
		if (sec == null) {
			mod.logE(TAG + "Section not found: " + Section.PROCRANK + " (ignoring section)");
		} else {
		...
		if (!ch.isEmpty()) {
			...
			mod.logE(TAG + "No usable sections found (aborting plugin)");
		}
	...
	private void generateMemoryInfoSec(Module mod, Chapter mainCh, Section sec) {
		...
		for (int i = 0; i < cnt; i++) {
			...
			try {
				...
				mod.logE(TAG + "Error parsing number: " + value);
				continue;
			}
			String expl = null;
	...
	private void generateProcrankSec(Module mod, Chapter ch, Section sec) {
		try {
			...
			mod.logE(TAG + "Failed gathering data from procrank output... Maybe procrank is buggy again?: " + e);
		}
	}

	private void generateProcrankSecUnsafe(Module mod_, Chapter mainCh, Section sec) {
		BugExpertModule mod = (BugExpertModule) mod_;
		if (sec.getLineCount() < 10) {
			// Suspiciously small...
			mod.logE(TAG + "procrank section is suspiciously small... ignoring it");
			return;
		}
		...
		if (showPerc) {
			t.addColumn("(%)", "The sum of Pss as percentage of total memory (available for android)",
		...
		if (showPerc) {
			t.addColumn("%", "The sum of Uss as percentage of total memory (available for android)",
					Table.FLAG_ALIGN_RIGHT);
		}
		t.begin();
	...
	private void loadServMeminfoSec(Module mod) {
		try {
			...
			mod.logE(TAG + "Failed gathering data from meminfo service output... Maybe output format has changed?: "
					+ e);
			mMemInfos.clear();
		}
	}
	...
	private void loadServMeminfoSecUnsafe(Module mod_) {
		...
		if (sec == null) {
			mod.logE(TAG + "Section not found: " + Section.DUMP_OF_SERVICE_MEMINFO + " (ignoring section)");
			return;
		}
	...
	private void loadLibrankSec(Module mod) {
		try {
			...
			mod.logE(TAG + "Failed gathering data from librank output... Maybe it's broken again?: " + e);
			mLRMemInfoMem.clear();
			mLRMemInfoPid.clear();
		}
	}
	...
	private void loadLibrankSecUnsafe(Module mod) {
		...
		if (s == null) {
			// Suspiciously small...
			mod.logE(TAG + "librank section is missing... ignoring it");
			return;
		}
		int cnt = s.getLineCount();
		if (cnt < 10) {
			// Suspiciously small...
			mod.logE(TAG + "librank section is suspiciously small... ignoring it");
			return;
		}
		String line = s.getLine(0);
		if (!line.equals(" RSStot      VSS      RSS      PSS      USS  Name/PID")) {
			mod.logE(TAG + "librank section format not supported... ignoring it");
			return;
		}
		String memName = null;
		for (int i = 1; i < cnt; i++) {
			...
			if (line.startsWith("        ")) {
				...
				if (memName == null) {
					mod.logE(TAG + "Parse error in librank output... trying to continue though (line: " + i + ")");
				} else {
					// Parse the data
	...
	private DocNode generateLibrankStat(BugExpertModule mod, LRMemInfoList list, int limit) {
		Block ret = new Block();
