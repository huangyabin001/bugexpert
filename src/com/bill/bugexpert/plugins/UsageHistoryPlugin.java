...
import com.bill.bugexpert.plugins.PackageInfoPlugin.PackageInfo;
import com.bill.bugexpert.plugins.logs.event.ActivityManagerStatsGenerator;
import com.bill.bugexpert.plugins.logs.event.ComponentStat;
...
public class UsageHistoryPlugin extends Plugin {
    ...
    public void load(Module br) {
        ...
        if (!mData.getName().equals("usage-history")) {
            br.logE(TAG + "Cannot parse section " + Section.USAGE_HISTORY + ": root tag invalid: " + mData.getName());
            return;
        }
        for (XMLNode pkg : mData) {
            if (pkg.getName() == null) continue; // ignore text
            if (!"pkg".equals(pkg.getName())) {
                br.logE(TAG + "Cannot parse section " + Section.USAGE_HISTORY + ": package tag invalid: " + pkg.getName());
                return;
            }
            ...
            for (XMLNode act : pkg) {
                ...
                if (!"comp".equals(act.getName())) {
                    br.logE(TAG + "Cannot parse section " + Section.USAGE_HISTORY + ": component tag invalid: " + act.getName());
                    return;
                }
