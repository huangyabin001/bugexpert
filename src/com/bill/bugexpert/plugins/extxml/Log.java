...
import com.bill.bugexpert.plugins.logs.LogMatcher;
...
/* package */ class Log {
    ...
    public Log(BugExpertModule mod, Chapter ch, XMLNode code) {
        mMod = mod;
        mCh = ch;
        mCode = code;
    }
    ...
    public void exec() {
        ...
        for (XMLNode node : mCode) {
            ...
            if (tag == null) {
                ...
                mMod.logE("Unknown tag in log: " + tag);
            }
        }
    ...
    class LogState {
        ...
        public LogState(String logName) {
            ...
            if (logs == null || logs.size() == 0) {
                mMod.logE("Log '" + logName + "' not found or empty!");
            } else {
                count = logs.size();
            }
        }
