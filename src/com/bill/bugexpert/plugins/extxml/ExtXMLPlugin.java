 ...
 */
package com.bill.bugexpert.plugins.extxml;
...
public class ExtXMLPlugin extends Plugin {
    ...
    public void hook(Module mod) {
        ...
        for (XMLNode hook : mXml) {
            ...
            if (into == null) {
                mod.logE("Missing 'into' attribute in hook tag, ignoring whole tag!");
                continue;
            }
            Plugin dst = mod.getPlugin(into);
            if (dst == null) {
                mod.logE("Cannot find plugin to hook into: " + into);
                continue;
            }
            dst.onHook(mod, hook);
        }
    ...
    public void load(Module mod) {
        ...
        for (XMLNode chTag : load) {
            ...
            mod.logE("Unknown tag is found in <load>, ignoreing it: " + tag);
        }
    }
    ...
    public void generate(Module mod) {
        ...
        for (XMLNode chTag : gen) {
            ...
            if ("chapter".equals(tag)) {
                ...
                mod.logE("A non-chapter tag is found in <generate>, ignoreing it: " + tag);
            }
        }
    }
    ...
    private void exec(Module mod, Chapter ch, XMLNode code) {
        ...
        if (type == null) {
            ...
            // Create a log
            new Log((BugExpertModule) mod, ch, code).exec();
        } else {
            mod.logE("Unknown code tag in logchart: " + code.getName());
        }
